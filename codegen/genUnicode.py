...
blockAliases.append(
	"PrivateUse:PrivateUseArea,SupplementaryPrivateUseArea-A,"
...
for name in ckeys:
	...
	if len(Categories[name]) > minTableSize:
		output.write("    return(xmlCharInRange((unsigned int)code, &xml%sG)" % name)
	else:
		start = 1
		for range in ranges:
			(begin, end) = range
			...
			if begin == end:
				...
				output.write("((code >= %s) && (code <= %s))" % (hex(begin), hex(end)))
	output.write(");\n}\n\n")
