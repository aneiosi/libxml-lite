...
for filename in sorted(glob.glob("../html5lib-tests/tokenizer/*.test")):
	...
	result_out = open(f"test/result/html-tokenizer/{testname}.test", "w")
	...
	for tests in root.values():
		for test in tests:
			...
			input = re.sub(r"\\u([A-Fa-f0-9]{4})", lambda m: chr(int(m[1], 16)), input)
			...
			output = re.sub(r"\\u([A-Fa-f0-9]{4})", lambda m: chr(int(m[1], 16)), output)
			...
			for state in test.get("initialStates", ["Data state"]):
				...
				test_out.write(f"{counter} {start_tag} {state_no} " f"{len(input.encode())}\n")
