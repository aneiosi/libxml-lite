...
for line in defines.readlines():
	...
	try:
		...
		if fields[0] == "name":
			name = fields[1]
			if state != 0:
				print("'name' %s found before previous name" "completed" % (fields[1]))
				continue
			state = 1
			if name in Functs:
				print("name '%s' already present - may give" " wrong results" % (name))
			else:
			...
			for el in fields[1:]:
				...
				if pos <= 0:
					...
					if (value < 0) | (value > 0x1FFFFF):
						raise Exception("Illegal value (%s) in ch for" " name %s" % (el, name))
				...
				if currange[1] < 0x100:  # single-byte
					...
					if currange in Functs[name][1]:
						raise Exception("range already defined in" " function")
					else:
...
for f in fkeys:
	if len(Functs[f][1]) > 0:  # only generate if unicode ranges present
		...
		group = rangetab.gen_range_tables(output, f, "_srng", "_lrng", rangeTable)
