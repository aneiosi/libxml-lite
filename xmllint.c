...
static void parsePath(xmllintState* lint, const xmlChar* path) {
	const xmlChar* cur;
...
static xmlParserErrors xmllintResourceLoader(
	void*               ctxt,
	const char*         URL,
	const char*         ID,
	...
	for (i = 0; i < lint->nbpaths; i++) {
		...
		if (newURL != NULL) {
			if (lint->defaultResourceLoader != NULL) {
				code = lint->defaultResourceLoader(NULL, (const char*)newURL, ID, type, flags, out);
			} else {
...
static xmlDocPtr parseXml(xmllintState* lint, const char* filename) {
	...
	if (lint->appOptions & XML_LINT_MEMORY) {
		...
		input
			= xmlNewInputFromMemory(filename, lint->memoryData, lint->memorySize, XML_INPUT_BUF_STATIC);
		if (input == NULL) {
	...
	if (strcmp(filename, "-") == 0) {
		doc = xmlCtxtReadFd(ctxt, STDIN_FILENO, "-", NULL, lint->parseOptions | XML_PARSE_UNZIP);
	} else {
		doc = xmlCtxtReadFile(ctxt, filename, NULL, lint->parseOptions | XML_PARSE_UNZIP);
	}
...
#ifdef LIBXML_HTML_ENABLED
static xmlDocPtr parseHtml(xmllintState* lint, const char* filename) {
	xmlParserCtxtPtr ctxt = lint->ctxt;
	xmlDocPtr        doc;
	...
	if (strcmp(filename, "-") == 0) {
		doc = htmlCtxtReadFd(ctxt, STDIN_FILENO, "-", NULL, lint->htmlOptions);
	} else {
...
static void getTime(xmlTime* time) {
#ifdef _WIN32
 ...
 */
static void startTimer(xmllintState* lint) {
	getTime(&lint->begin);
}
 ...
 *           type argument
 */
static void LIBXML_ATTR_FORMAT(2, 3) endTimer(xmllintState* lint, const char* fmt, ...) {
	xmlSeconds msec;
	va_list    ap;
...
static void internalSubsetDebug(
	void* ctx, const xmlChar* name, const xmlChar* ExternalID, const xmlChar* SystemID
) {
	xmllintState* lint = ctx;
...
static void externalSubsetDebug(
	void* ctx, const xmlChar* name, const xmlChar* ExternalID, const xmlChar* SystemID
) {
	xmllintState* lint = ctx;
...
static void attributeDeclDebug(
	void*             ctx,
	const xmlChar*    elem,
	const xmlChar*    name,
	int               type,
	...
	if (defaultValue == NULL) {
		...
		fprintf(
			stdout, "SAX.attributeDecl(%s, %s, %d, %d, %s, ...)\n", elem, name, type, def, defaultValue
		);
	}
	xmlFreeEnumeration(tree);
}

static void elementDeclDebug(
	void* ctx, const xmlChar* name, int type, xmlElementContentPtr content ATTRIBUTE_UNUSED
) {
	xmllintState* lint = ctx;
...
static void unparsedEntityDeclDebug(
	void*          ctx,
	const xmlChar* name,
	const xmlChar* publicId,
	const xmlChar* systemId,
...
static void setDocumentLocatorDebug(void* ctx, xmlSAXLocatorPtr loc ATTRIBUTE_UNUSED) {
	xmllintState* lint = ctx;
...
static void startElementDebug(void* ctx, const xmlChar* name, const xmlChar** atts) {
	xmllintState* lint = ctx;
	int           i;
...
static void endElementDebug(void* ctx, const xmlChar* name) {
	xmllintState* lint = ctx;
...
static void processingInstructionDebug(void* ctx, const xmlChar* target, const xmlChar* data) {
	xmllintState* lint = ctx;
...
static void LIBXML_ATTR_FORMAT(2, 3) warningDebug(void* ctx, const char* msg, ...) {
	xmllintState* lint = ctx;
	va_list       args;
...
static void LIBXML_ATTR_FORMAT(2, 3) errorDebug(void* ctx, const char* msg, ...) {
	xmllintState* lint = ctx;
	va_list       args;
...
static void LIBXML_ATTR_FORMAT(2, 3) fatalErrorDebug(void* ctx, const char* msg, ...) {
	xmllintState* lint = ctx;
	va_list       args;
...
static void startElementNsDebug(
	void*           ctx,
	...
	if (attributes != NULL) {
		for (i = 0; i < nb_attributes * 5; i += 5) {
			...
			fprintf(
				stdout, "%.4s...', %d", attributes[i + 3], (int)(attributes[i + 4] - attributes[i + 3])
			);
		}
	}
	fprintf(stdout, ")\n");
...
static void
endElementNsDebug(void* ctx, const xmlChar* localname, const xmlChar* prefix, const xmlChar* URI) {
	xmllintState* lint = ctx;
...
static void testSAX(xmllintState* lint, const char* filename) {
	lint->callbacks = 0;
...
static void processNode(xmllintState* lint, xmlTextReaderPtr reader) {
	...
	if (lint->patternc) {
		...
		if (type == XML_READER_TYPE_ELEMENT) {
			...
			match = xmlPatternMatch(lint->patternc, xmlTextReaderCurrentNode(reader));
		...
		if (lint->patstream != NULL) {
			...
			if (type == XML_READER_TYPE_ELEMENT) {
				...
				if (ret < 0) {
					...
					fprintf(lint->errStream, "xmlPatternMatch and xmlStreamPush disagree\n");
					if (path != NULL) {
...
static void streamFile(xmllintState* lint, const char* filename) {
	...
	if (lint->appOptions & XML_LINT_MEMORY) {
		reader
			= xmlReaderForMemory(lint->memoryData, lint->memorySize, filename, NULL, lint->parseOptions);
		if (reader == NULL) {
			lint->progresult = XMLLINT_ERR_MEM;
			return;
		}
	...
	#endif
	{
		...
		if (strcmp(filename, "-") == 0) {
			reader = xmlReaderForFd(STDIN_FILENO, "-", NULL, lint->parseOptions | XML_PARSE_UNZIP);
		} else {
			reader = xmlReaderForFile(filename, NULL, lint->parseOptions | XML_PARSE_UNZIP);
		}
		if (reader == NULL) {
	...
	if (lint->parseOptions & XML_PARSE_DTDVALID) {
		if (xmlTextReaderIsValid(reader) != 1) {
			fprintf(errStream, "Document %s does not validate\n", filename);
			lint->progresult = XMLLINT_ERR_VALID;
		}
	}
...
static void walkDoc(xmllintState* lint, xmlDocPtr doc) {
	...
	if (lint->pattern != NULL) {
		...
		if (root == NULL) {
			fprintf(errStream, "Document does not have a root element");
			lint->progresult = XMLLINT_ERR_UNCLASS;
			return;
		}
		...
		ret             = xmlPatternCompileSafe(
      (const xmlChar*)lint->pattern, doc->dict, 0, &namespaces[0], &lint->patternc
    );
		if (lint->patternc == NULL) {
			if (ret < 0) {
				lint->progresult = XMLLINT_ERR_MEM;
...
static void doXPathDump(xmllintState* lint, xmlXPathObjectPtr cur) {
	switch (cur->type) {
		case XPATH_NODESET: {
	#ifdef LIBXML_OUTPUT_ENABLED
...
static void doXPathQuery(xmllintState* lint, xmlDocPtr doc, const char* query) {
...
static xmlDocPtr parseFile(xmllintState* lint, const char* filename) {
	xmlDocPtr doc = NULL;
	...
	if (doc == NULL) {
		...
		if ((lint->parseOptions & XML_PARSE_DTDVALID) && (status & XML_STATUS_DTD_VALIDATION_FAILED)) {
			lint->progresult = XMLLINT_ERR_VALID;
		}
...
static void parseAndPrintFile(xmllintState* lint, const char* filename) {
	FILE*     errStream = lint->errStream;
	xmlDocPtr doc;
	...
	if (
		...
		if (doc->children != NULL) {
			node = doc->children;
			while ((node != NULL) && ((node->type != XML_ELEMENT_NODE) || (node->last == NULL))) {
				node = node->next;
			}
			if (node != NULL) {
	...
	if (lint->noout == 0) {
		...
		if ((lint->appOptions & XML_LINT_DEBUG_ENABLED) != XML_LINT_DEBUG_ENABLED) {
	...
	#endif
			{
				...
				if (lint->output == NULL) {
					ctxt = xmlSaveToFd(STDOUT_FILENO, lint->encoding, saveOpts);
				} else {
					ctxt = xmlSaveToFilename(lint->output, lint->encoding, saveOpts);
				}
	...
	if ((lint->dtdvalid != NULL) || (lint->dtdvalidfpi != NULL)) {
		...
		if (dtd == NULL) {
			...
			if (!xmlValidateDtd(cvp, doc, dtd)) {
				if (lint->dtdvalid != NULL) {
					fprintf(
						errStream, "Document %s does not validate against %s\n", filename, lint->dtdvalid
					);
				} else {
					fprintf(
...
#endif /* LIBXML_VALID_ENABLED */
	fprintf(
		f, "\t--strict-namespace : Return application failure if document has any namespace errors\n"
	);
...
static int skipArgs(const char* arg) {
	if ((!strcmp(arg, "-path")) || (!strcmp(arg, "--path")) || (!strcmp(arg, "-maxmem"))
	    || (!strcmp(arg, "--maxmem")) ||
#ifdef LIBXML_OUTPUT_ENABLED
	    (!strcmp(arg, "-o")) || (!strcmp(arg, "-output")) || (!strcmp(arg, "--output"))
	    || (!strcmp(arg, "-encode")) || (!strcmp(arg, "--encode")) || (!strcmp(arg, "-pretty"))
	    || (!strcmp(arg, "--pretty")) ||
#endif
#ifdef LIBXML_VALID_ENABLED
	    (!strcmp(arg, "-dtdvalid")) || (!strcmp(arg, "--dtdvalid")) || (!strcmp(arg, "-dtdvalidfpi"))
	    || (!strcmp(arg, "--dtdvalidfpi")) ||
#endif
#ifdef LIBXML_RELAXNG_ENABLED
	    (!strcmp(arg, "-relaxng")) || (!strcmp(arg, "--relaxng")) ||
#endif
#ifdef LIBXML_SCHEMAS_ENABLED
	    (!strcmp(arg, "-schema")) || (!strcmp(arg, "--schema")) ||
#endif
#if defined(LIBXML_READER_ENABLED) && defined(LIBXML_PATTERN_ENABLED)
	    (!strcmp(arg, "-pattern")) || (!strcmp(arg, "--pattern")) ||
#endif
#ifdef LIBXML_XPATH_ENABLED
	    (!strcmp(arg, "-xpath")) || (!strcmp(arg, "--xpath")) ||
#endif
	    (!strcmp(arg, "-max-ampl")) || (!strcmp(arg, "--max-ampl"))) {
		return (1);
	}
...
static void xmllintInit(xmllintState* lint) {
...
static void xmllintOptWarnNoSupport(FILE* errStream, const char* opt, const char* nosupp) {
	fprintf(errStream, "Warning: Option %s doesn't support %s\n", opt, nosupp);
}

static int xmllintParseOptions(xmllintState* lint, int argc, const char** argv) {
	FILE*       errStream   = lint->errStream;
	...
	for (i = 1; i < argc; i++) {
		...
		if ((!strcmp(argv[i], "-maxmem")) || (!strcmp(argv[i], "--maxmem"))) {
			...
			if (parseInteger(&val, errStream, "maxmem", argv[i], 0, INT_MAX) < 0) {
				return (XMLLINT_ERR_UNCLASS);
			}
			lint->maxmem = val;
		} else if ((!strcmp(argv[i], "-debug")) || (!strcmp(argv[i], "--debug"))) {
			lint->appOptions |= XML_LINT_DEBUG_ENABLED;
		} else if ((!strcmp(argv[i], "-shell")) || (!strcmp(argv[i], "--shell"))) {
			lint->appOptions |= XML_LINT_NAVIGATING_SHELL;
		} else if ((!strcmp(argv[i], "-copy")) || (!strcmp(argv[i], "--copy"))) {
			lint->appOptions |= XML_LINT_COPY_ENABLED;
		} else if ((!strcmp(argv[i], "-recover")) || (!strcmp(argv[i], "--recover"))) {
			...
			if (parseInteger(&val, errStream, "pretty", argv[i], 0, 2) < 0) {
				return (XMLLINT_ERR_UNCLASS);
			}
			lint->format = val;
	#ifdef LIBXML_ZLIB_ENABLED
		...
		} else if ((!strcmp(argv[i], "-max-ampl")) || (!strcmp(argv[i], "--max-ampl"))) {
			i++;
			if (i >= argc) {
				fprintf(errStream, "max-ampl: missing integer value\n");
				return (XMLLINT_ERR_UNCLASS);
			}
			if (parseInteger(&val, errStream, "max-ampl", argv[i], 1, UINT_MAX) < 0) {
				return (XMLLINT_ERR_UNCLASS);
			}
			lint->maxAmpl = val;
		} else {
	...
	if (specialMode != NULL) {
		...
		if (lint->format > 0) {
			xmllintOptWarnNoSupport(errStream, specialMode, "--format or -pretty");
		}
	#ifdef LIBXML_ZLIB_ENABLED
		if (lint->appOptions & XML_LINT_ZLIB_COMPRESSION) {
	...
	if (lint->pattern
	    && !(
				(lint->appOptions & XML_LINT_USE_STREAMING) || (lint->appOptions & XML_LINT_USE_WALKER)
			)) {
...
int xmllintMain(int argc, const char** argv, FILE* errStream, xmlResourceLoader loader) {
...
#ifdef LIBXML_RELAXNG_ENABLED
	if (
		(lint->relaxng != NULL) && ((lint->appOptions & XML_LINT_SAX_ENABLED) != XML_LINT_SAX_ENABLED)
	#ifdef LIBXML_READER_ENABLED
...
#if defined(LIBXML_READER_ENABLED) && defined(LIBXML_PATTERN_ENABLED)
	if ((lint->pattern != NULL)
	    && ((lint->appOptions & XML_LINT_USE_WALKER) != XML_LINT_USE_WALKER)) {
		res = xmlPatternCompileSafe(BAD_CAST lint->pattern, NULL, 0, NULL, &lint->patternc);
		if (lint->patternc == NULL) {
			if (res < 0) {
				lint->progresult = XMLLINT_ERR_MEM;
			} else {
	...
	for (i = 1; i < argc; i++) {
		...
		if (lint->appOptions & XML_LINT_MEMORY) {
			...
			lint->memoryData = mmap(NULL, info.st_size, PROT_READ, MAP_SHARED, memoryFd, 0);
	...
	#ifdef LIBXML_PUSH_ENABLED
				if (lint->appOptions & XML_LINT_PUSH_ENABLED) {
					ctxt = htmlCreatePushParserCtxt(NULL, NULL, NULL, 0, filename, XML_CHAR_ENCODING_NONE);
				} else
	#endif /* LIBXML_PUSH_ENABLED */
				{
	...
	if ((files == 0) && ((lint->appOptions & XML_LINT_GENERATE) != XML_LINT_GENERATE)
	    && (lint->version == 0)) {
		usage(errStream, argv[0]);
		lint->progresult = XMLLINT_ERR_UNCLASS;
	}
...
#ifdef FUZZING_BUILD_MODE_UNSAFE_FOR_PRODUCTION
	if ((lint->maxmem) && (xmllintOom != (lint->progresult == XMLLINT_ERR_MEM))) {
