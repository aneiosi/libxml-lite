...
#define NODE_MASK_TEXT_CONTENT                                                    \
	((1 << XML_TEXT_NODE) | (1 << XML_CDATA_SECTION_NODE) | (1 << XML_COMMENT_NODE) \
	 | (1 << XML_PI_NODE))

#define CHILD_MASK_DOCUMENT                                                \
	((1 << XML_ELEMENT_NODE) | (1 << XML_PI_NODE) | (1 << XML_COMMENT_NODE))

#define CHILD_MASK_CONTENT                                                        \
	((1 << XML_ELEMENT_NODE) | (1 << XML_TEXT_NODE) | (1 << XML_CDATA_SECTION_NODE) \
	 | (1 << XML_ENTITY_REF_NODE) | (1 << XML_PI_NODE) | (1 << XML_COMMENT_NODE))

#define CHILD_MASK_ELEMENT   (CHILD_MASK_CONTENT | (1 << XML_ATTRIBUTE_NODE))

#define CHILD_MASK_ATTRIBUTE ((1 << XML_TEXT_NODE) | (1 << XML_ENTITY_REF_NODE))

#define CHILD_MASK_DTD                                                           \
	((1 << XML_ELEMENT_DECL) | (1 << XML_ATTRIBUTE_DECL) | (1 << XML_ENTITY_DECL))
...
static void setInt(int offset, int n) {
	int idx             = (vars->intIdx - offset - 1) & REG_MASK;
	vars->integers[idx] = n;
}

static void incIntIdx(void) {
	vars->intIdx = (vars->intIdx + 1) & REG_MASK;
}

/* Strings */

static const xmlChar* getStr(int offset) {
...
static const char* getCStr(int offset) {
	return (const char*)getStr(offset);
}

static void setStr(int offset, xmlChar* str) {
...
static void moveStr(int offset, xmlChar* str) {
 ...
 * injection.
 */
static xmlChar* uncheckedStrndup(const xmlChar* str, int size) {
	xmlChar* copy;
...
static xmlChar* uncheckedStrdup(const xmlChar* str) {
	return uncheckedStrndup(str, MAX_CONTENT);
}

static void copyStr(int offset, const xmlChar* str) {
	setStr(offset, uncheckedStrdup(str));
}

static void incStrIdx(void) {
	vars->stringIdx = (vars->stringIdx + 1) & REG_MASK;
}

/* Nodes */

static void       dropNode(xmlNodePtr node);

static xmlNodePtr getNode(int offset) {
...
static xmlDocPtr getDoc(int offset) {
...
static xmlAttrPtr getAttr(int offset) {
	xmlNodePtr node = getNode(offset);
...
static xmlDtdPtr getDtd(int offset) {
	xmlNodePtr node = getNode(offset);
	xmlDocPtr  doc;
...
static void setNode(int offset, xmlNodePtr node) {
	int        idx     = (vars->nodeIdx - offset - 1) & REG_MASK;
...
static int isValidChildType(xmlNodePtr parent, int childType) {
	return ((1 << childType) & childMasks[parent->type]) != 0;
}

static int isValidChild(xmlNodePtr parent, xmlNodePtr child) {
	xmlNodePtr cur;
...
static int isTextContentNode(xmlNodePtr child) {
	if (child == NULL) {
		return 0;
	}
...
static int isDtdChild(xmlNodePtr child) {
	if (child == NULL) {
		return 0;
	}
...
static xmlNodePtr nodeGetTree(xmlNodePtr node) {
	xmlNodePtr cur = node;
...
static void dropNode(xmlNodePtr node) {
	...
	if (tree != (xmlNodePtr)doc && !isDtdChild(tree)) {
		if (doc == NULL || tree->type != XML_DTD_NODE
		    || ((xmlDtdPtr)tree != doc->intSubset && (xmlDtdPtr)tree != doc->extSubset)) {
			xmlFreeNode(tree);
		}
	}
...
static void removeNode(xmlNodePtr node) {
	int i;
...
static void removeChildren(xmlNodePtr parent, int self) {
	int i;
...
static xmlNsPtr nodeGetNs(xmlNodePtr node, int k) {
	int      i = 0;
	xmlNsPtr ns, next;
...
static void checkContent(xmlNodePtr node) {
	if (node != NULL
	    && (node->type == XML_TEXT_NODE || node->type == XML_CDATA_SECTION_NODE || node->type == XML_ENTITY_NODE || node->type == XML_PI_NODE || node->type == XML_COMMENT_NODE || node->type == XML_NOTATION_NODE)
...
static int countNodes(xmlNodePtr node) {
	xmlNodePtr cur;
	int        numNodes;
...
static xmlNodePtr checkCopy(xmlNodePtr copy) {
	vars->numCopyOps += 1;

	if (copy != NULL && (vars->numCopyOps > MAX_COPY_OPS || countNodes(copy) > MAX_COPY_NODES)) {
		if (copy->type == XML_DOCUMENT_NODE || copy->type == XML_HTML_DOCUMENT_NODE) {
 ...
 * nodes.
 */
static int fixNs(xmlNodePtr node) {
	if (node == NULL) {
		return 0;
	}
...
static void opNodeAccessor(int op) {
	xmlNodePtr node;
	...
	switch (op) {
		...
		case OP_XML_GET_LAST_CHILD:
			startOp("xmlGetLastChild");
			break;
		case OP_XML_GET_INT_SUBSET:
			startOp("xmlGetIntSubset");
			break;
		case OP_XML_DOC_GET_ROOT_ELEMENT:
			startOp("xmlDocGetRootElement");
			break;
		default:
			break;
	}
	...
	if (node != NULL) {
		switch (op) {
			...
			case OP_XML_GET_LAST_CHILD:
				...
				break;
			case OP_XML_GET_INT_SUBSET:
				node = (xmlNodePtr)xmlGetIntSubset(node->doc);
				break;
			case OP_XML_DOC_GET_ROOT_ELEMENT:
				node = xmlDocGetRootElement(node->doc);
				break;
			default:
				break;
		}
	...
	while (xmlFuzzBytesRemaining()) {
		...
		switch (op) {
			...
			case OP_PARSE_DOCUMENT:
				...
				startOp("xmlReadDoc");
				incNodeIdx();
				setNode(0, (xmlNodePtr)xmlReadDoc(getStr(0), getCStr(1), getCStr(2), getInt(0)));
				break;
			...
			case OP_XML_NEW_NODE: {
				...
				startOp("xmlNewNode");
				incNodeIdx();
				node      = xmlNewNode(nodeGetNs(getNode(1), getInt(0)), name = getStr(0));
			...
			case OP_XML_NEW_NODE_EAT_NAME: {
				...
				incNodeIdx();
				node
					= xmlNewNodeEatName(nodeGetNs(getNode(1), getInt(0)), name = uncheckedStrdup(getStr(0)));
				oomReport = (name != NULL && node == NULL);
				if (fixNs(node) < 0) {
					oomReport = 1;
				}
			...
			case OP_XML_NEW_DOC_NODE: {
				...
				incNodeIdx();
				node = xmlNewDocNode(
					getDoc(1), nodeGetNs(getNode(2), getInt(0)), name = getStr(0), getStr(1)
				);
				oomReport = (name != NULL && node == NULL);
				if (fixNs(node) < 0) {
					oomReport = 1;
			...
			case OP_XML_NEW_DOC_NODE_EAT_NAME: {
				...
				node = xmlNewDocNodeEatName(
					getDoc(1), nodeGetNs(getNode(2), getInt(0)), name = uncheckedStrdup(getStr(0)), getStr(1)
				);
				oomReport = (name != NULL && node == NULL);
				if (fixNs(node) < 0) {
					oomReport = 1;
			...
			case OP_XML_NEW_CHILD: {
				...
				node = xmlNewChild(
					parent = getNode(1), nodeGetNs(getNode(1), getInt(0)), name = getStr(0), getStr(1)
				);
				oomReport
					= (parent != NULL && isValidChildType(parent, XML_ELEMENT_NODE) && name != NULL && node == NULL);
				setNode(0, node);
				break;
			}
			...
			case OP_XML_NEW_PROP: {
				...
				startOp("xmlNewProp");
				incNodeIdx();
				attr = xmlNewProp(parent = getNode(1), name = getStr(0), getStr(1));
				oomReport
					= ((parent == NULL || parent->type == XML_ELEMENT_NODE) && name != NULL && attr == NULL);
			...
			case OP_XML_NEW_DOC_PROP: {
				...
				incNodeIdx();
				attr      = xmlNewDocProp(getDoc(1), name = getStr(0), getStr(1));
			...
			case OP_XML_NEW_NS_PROP: {
				...
				incNodeIdx();
				attr = xmlNewNsProp(getNode(1), nodeGetNs(getNode(1), getInt(0)), getStr(0), getStr(1));
			...
			case OP_XML_NEW_NS_PROP_EAT_NAME: {
				...
				attr = xmlNewNsPropEatName(
					getNode(1), nodeGetNs(getNode(1), getInt(0)), uncheckedStrdup(getStr(0)), getStr(1)
				);
				if (attr != NULL) {
					oomReport = 0;
				}
			...
			case OP_XML_NEW_PI: {
				...
				startOp("xmlNewPI");
				incNodeIdx();
				node      = xmlNewPI(name = getStr(0), getStr(1));
				oomReport = (name != NULL && node == NULL);
				setNode(0, node);
				break;
			}
			...
			case OP_XML_NEW_DOC_PI: {
				...
				startOp("xmlNewDocPI");
				incNodeIdx();
				node      = xmlNewDocPI(getDoc(1), name = getStr(0), getStr(1));
				oomReport = (name != NULL && node == NULL);
				setNode(0, node);
				break;
			}
			...
			case OP_XML_NEW_DOC_COMMENT: {
				...
				incNodeIdx();
				node      = xmlNewDocComment(getDoc(1), getStr(0));
				oomReport = (node == NULL);
				setNode(0, node);
				break;
			}
			...
			case OP_XML_NEW_CHAR_REF: {
				...
				incNodeIdx();
				node      = xmlNewCharRef(getDoc(1), name = getStr(0));
				oomReport = (name != NULL && node == NULL);
				setNode(0, node);
				break;
			}
			...
			case OP_XML_NEW_REFERENCE: {
				...
				incNodeIdx();
				node      = xmlNewReference(getDoc(1), name = getStr(0));
				oomReport = (name != NULL && node == NULL);
				setNode(0, node);
				break;
			}
			...
			case OP_XML_CREATE_INT_SUBSET: {
				...
				if (doc == NULL || doc->intSubset == NULL) {
					dtd       = xmlCreateIntSubset(doc, getStr(0), getStr(1), getStr(2));
					oomReport = (dtd == NULL);
				}
			...
			case OP_XML_NEW_DTD: {
				...
				if (doc == NULL || doc->extSubset == NULL) {
					dtd       = xmlNewDtd(doc, getStr(0), getStr(1), getStr(2));
					oomReport = (dtd == NULL);
				}
				setNode(0, (xmlNodePtr)dtd);
				break;
			...
			case OP_XML_COPY_DOC: {
				...
				startOp("xmlCopyDoc");
				incNodeIdx();
				copy = xmlCopyDoc(getDoc(1), getInt(0));
				/*
			...
			case OP_XML_COPY_NODE: {
				...
				startOp("xmlCopyNode");
				incNodeIdx();
				copy = xmlCopyNode(getNode(1), getInt(0));
				if (copy != NULL) {
					oomReport = 0;
				}
				setNode(0, checkCopy((xmlNodePtr)copy));
			...
			case OP_XML_DOC_COPY_NODE: {
				...
				incNodeIdx();
				copy = xmlDocCopyNode(node = getNode(1), doc = getDoc(2), getInt(0));
				if (copy != NULL) {
					oomReport = 0;
				}
				setNode(0, checkCopy((xmlNodePtr)copy));
			...
			case OP_XML_DOC_COPY_NODE_LIST: {
				...
				copy = xmlDocCopyNodeList(getDoc(0), getNode(1));
				if (copy != NULL) {
					oomReport = 0;
				}
				xmlFreeNodeList(copy);
			...
			case OP_XML_COPY_PROP: {
				...
				startOp("xmlCopyProp");
				incNodeIdx();
				copy = xmlCopyProp(getNode(1), getAttr(2));
				/*
			...
			case OP_XML_COPY_PROP_LIST: {
				...
				copy = xmlCopyPropList(getNode(0), getAttr(1));
				if (copy != NULL) {
					oomReport = 0;
				}
			...
			case OP_XML_NODE_SET_NAME:
				...
				xmlNodeSetName(getNode(0), getStr(0));
				endOp();
				break;
			...
			case OP_XML_NODE_SET_CONTENT: {
				...
				res       = xmlNodeSetContent(node, getStr(0));
				oomReport = (res < 0);
				endOp();
				break;
			}
			...
			case OP_XML_NODE_SET_CONTENT_LEN: {
				...
				content = getStr(0);
				removeChildren(node, 0);
				res       = xmlNodeSetContentLen(node, content, xmlStrlen(content));
				oomReport = (res < 0);
				endOp();
				break;
			}
			...
			case OP_XML_NODE_ADD_CONTENT: {
				...
				node      = getNode(0);
				res       = xmlNodeAddContent(node, getStr(0));
				oomReport = (res < 0);
				if (node != NULL) {
					if (node->type == XML_ELEMENT_NODE || node->type == XML_DOCUMENT_FRAG_NODE) {
						text = node->last;
					} else {
						text = node;
					}
			...
			case OP_XML_NODE_ADD_CONTENT_LEN: {
				...
				node      = getNode(0);
				content   = getStr(0);
				res       = xmlNodeAddContentLen(node, content, xmlStrlen(content));
				oomReport = res < 0;
				if (node != NULL) {
					if (node->type == XML_ELEMENT_NODE || node->type == XML_DOCUMENT_FRAG_NODE) {
						text = node->last;
			...
			case OP_XML_DOC_SET_ROOT_ELEMENT: {
				...
				if (oldRoot != NULL || root == NULL || root->doc == oldDoc) {
					oomReport = 0;
				}
				setNode(0, oldRoot);

				if (root && (root->parent != oldParent || root->doc != oldDoc)) {
					if (fixNs(root) < 0) {
						oomReport = 1;
					}
					if (oldParent != NULL) {
			...
			case OP_XML_NODE_GET_ATTR_VALUE: {
				...
				res       = xmlNodeGetAttrValue(getNode(0), getStr(1), getStr(2), &value);
				oomReport = (res < 0);
				moveStr(0, value);
				endOp();
				break;
			...
			case OP_XML_NODE_SET_LANG: {
				...
				node = getNode(0);
				attr = xmlHasNsProp(node, BAD_CAST "lang", XML_XML_NAMESPACE);
				xmlFuzzResetFailure();
				removeChildren((xmlNodePtr)attr, 0);
				res       = xmlNodeSetLang(node, getStr(0));
				oomReport = (res < 0);
				endOp();
				break;
			}
			...
			case OP_XML_NODE_SET_SPACE_PRESERVE: {
				...
				node = getNode(0);
				attr = xmlHasNsProp(node, BAD_CAST "space", XML_XML_NAMESPACE);
				xmlFuzzResetFailure();
				removeChildren((xmlNodePtr)attr, 0);
				res       = xmlNodeSetSpacePreserve(node, getInt(0));
				oomReport = (res < 0);
				endOp();
				break;
			}
			...
			case OP_XML_NODE_GET_BASE: {
				...
				base = xmlNodeGetBase(getDoc(0), getNode(1));
				if (base != NULL) {
					oomReport = 0;
				}
				moveStr(0, base);
			...
			case OP_XML_NODE_GET_BASE_SAFE: {
				...
				incStrIdx();
				res       = xmlNodeGetBaseSafe(getDoc(0), getNode(1), &base);
				oomReport = (res < 0);
				moveStr(0, base);
				endOp();
				break;
			...
			case OP_XML_NODE_SET_BASE: {
				...
				node = getNode(0);
				attr = xmlHasNsProp(node, BAD_CAST "base", XML_XML_NAMESPACE);
				xmlFuzzResetFailure();
				removeChildren((xmlNodePtr)attr, 0);
				res = xmlNodeSetBase(node, getStr(0));
				if (res == 0) {
					oomReport = 0;
				}
				endOp();
			...
			case OP_XML_HAS_PROP: {
				...
				startOp("xmlHasProp");
				incNodeIdx();
				attr = xmlHasProp(node = getNode(1), getStr(0));
				if (node != NULL && node->doc != NULL && node->doc->intSubset != NULL) {
					/*
			...
			case OP_XML_HAS_NS_PROP: {
				...
				incNodeIdx();
				attr = xmlHasNsProp(node = getNode(1), getStr(0), getStr(1));
				if (node != NULL && node->doc != NULL && node->doc->intSubset != NULL) {
					if (attr != NULL) {
						oomReport = 0;
					}
			...
			case OP_XML_GET_PROP: {
				...
				startOp("xmlGetProp");
				incStrIdx();
				content = xmlGetProp(getNode(0), getStr(1));
				if (content != NULL) {
					oomReport = 0;
				}
				moveStr(0, content);
			...
			case OP_XML_GET_NS_PROP: {
				...
				incStrIdx();
				content = xmlGetNsProp(getNode(0), getStr(1), getStr(2));
				if (content != NULL) {
					oomReport = 0;
				}
				moveStr(0, content);
			...
			case OP_XML_GET_NO_NS_PROP: {
				...
				incStrIdx();
				content = xmlGetNoNsProp(getNode(0), getStr(1));
				if (content != NULL) {
					oomReport = 0;
				}
				moveStr(0, content);
			...
			case OP_XML_SET_PROP: {
				...
				oomReport
					= (node != NULL && node->type == XML_ELEMENT_NODE && name != NULL && attr == NULL);
				setNode(0, (xmlNodePtr)attr);
				break;
			}
			...
			case OP_XML_SET_NS_PROP: {
				...
				oomReport
					= ((node == NULL || node->type == XML_ELEMENT_NODE) && (ns == NULL || ns->href != NULL) && name != NULL && attr == NULL);
				setNode(0, (xmlNodePtr)attr);
				if (ns != NULL) {
			...
			case OP_XML_REMOVE_PROP: {
				...
				attr = getNode(0);
				if (attr != NULL) {
					if (attr->parent != NULL && attr->type == XML_ATTRIBUTE_NODE) {
			...
			case OP_XML_NEW_NS: {
				...
				startOp("xmlNewNs");
				ns = xmlNewNs(node = getNode(0), getStr(0), getStr(1));
				if (ns != NULL) {
					oomReport = 0;
				}
				if (node == NULL) {
			...
			case OP_XML_SEARCH_NS: {
				...
				startOp("xmlSearchNs");
				ns = xmlSearchNs(getDoc(0), getNode(1), getStr(0));
				if (ns != NULL) {
					oomReport = 0;
				}
				endOp();
			...
			case OP_XML_SEARCH_NS_BY_HREF: {
				...
				ns = xmlSearchNsByHref(getDoc(0), getNode(1), getStr(0));
				if (ns != NULL) {
					oomReport = 0;
				}
				endOp();
			...
			case OP_XML_GET_NS_LIST: {
				...
				list = xmlGetNsList(getDoc(0), getNode(1));
				if (list != NULL) {
					oomReport = 0;
				}
				xmlFree(list);
			...
			case OP_XML_GET_NS_LIST_SAFE: {
				...
				startOp("xmlGetNsList");
				res       = xmlGetNsListSafe(getDoc(0), getNode(1), &list);
				oomReport = (res < 0);
				xmlFree(list);
				endOp();
				break;
			...
			case OP_XML_SET_NS: {
				...
				startOp("xmlSetNs");
				node = getNode(0), ns = nodeGetNs(getNode(1), getInt(0));
				xmlSetNs(node, ns);
				oomReport = 0;
				if (ns != NULL) {
					if (fixNs(node) < 0) {
			...
			case OP_XML_COPY_NAMESPACE: {
				...
				copy      = xmlCopyNamespace(ns = nodeGetNs(getNode(0), getInt(0)));
			...
			case OP_XML_COPY_NAMESPACE_LIST: {
				...
				copy      = xmlCopyNamespaceList(list = nodeGetNs(getNode(0), getInt(0)));
			...
			case OP_XML_REPLACE_NODE: {
				...
				oomReport
					= (old != NULL && old->parent != NULL && node != NULL && old != node && result == NULL);
				...
				if (node == NULL) {
					...
					dropNode(oldParent);
				} else if (node->parent != oldNodeParent || node->doc != oldNodeDoc) {
					if (fixNs(node) < 0) {
						oomReport = 1;
					}
			...
			case OP_XML_ADD_NEXT_SIBLING: {
				...
				switch (op) {
					case OP_XML_ADD_CHILD:
						...
						break;
					case OP_XML_ADD_SIBLING:
						startOp("xmlAddSibling");
						break;
					case OP_XML_ADD_PREV_SIBLING:
						startOp("xmlAddPrevSibling");
						break;
					case OP_XML_ADD_NEXT_SIBLING:
						startOp("xmlAddNextSibling");
						break;
				}
				...
				if (isDtdChild(node) || !isValidChild(parent, node)) {
					node = NULL;
				}
				...
				if (node != NULL && node->type == XML_ATTRIBUTE_NODE) {
					if ((op == OP_XML_ADD_CHILD)
					    || (target != NULL && (target->type == XML_ATTRIBUTE_NODE))) {
						xmlAttrPtr attr = xmlHasNsProp(parent, node->name, node->ns ? node->ns->href : NULL);
				...
				} else if (target != NULL && target->type == XML_ATTRIBUTE_NODE) {
					node = NULL;
				}

				oldNodeParent = node ? node->parent : NULL;
				oldNodeDoc    = node ? node->doc : NULL;
				argsOk        = (target != NULL && node != NULL && target != node);
				...
				} else if (node != NULL && (node->parent != oldNodeParent || node->doc != oldNodeDoc)) {
					if (fixNs(node) < 0) {
						oomReport = 1;
					}
					/* Drop old parent of node */
			...
			case OP_XML_TEXT_CONCAT: {
				...
				node      = getNode(0);
				text      = getStr(0);
				res       = xmlTextConcat(node, text, xmlStrlen(text));
			...
			case OP_XML_STRING_GET_NODE_LIST: {
				...
				list      = xmlStringGetNodeList(getDoc(0), value = getStr(0));
			...
			case OP_XML_STRING_LEN_GET_NODE_LIST: {
				...
				doc       = getDoc(0);
				value     = getStr(0);
				list      = xmlStringLenGetNodeList(doc, value, xmlStrlen(value));
				oomReport = (value != NULL && value[0] != 0 && list == NULL);
				xmlFreeNodeList(list);
				endOp();
				break;
			...
			case OP_XML_NODE_LIST_GET_STRING: {
				...
				doc       = getDoc(0);
				list      = getNode(1);
				string    = xmlNodeListGetString(doc, list, getInt(0));
			...
			case OP_XML_NODE_LIST_GET_RAW_STRING: {
				...
				doc       = getDoc(0);
				list      = getNode(1);
				string    = xmlNodeListGetRawString(doc, list, getInt(0));
				oomReport = (list != NULL && string == NULL);
				moveStr(0, string);
				endOp();
				break;
			...
			case OP_XML_IS_XHTML:
				startOp("xmlIsXHTML");
				incIntIdx();
				setInt(0, xmlIsXHTML(getStr(0), getStr(1)));
				oomReport = 0;
				break;
			...
			case OP_XML_ADD_ELEMENT_DECL: {
				...
				incNodeIdx();
				decl = xmlAddElementDecl(NULL, getDtd(1), getStr(0), (xmlElementTypeVal)getInt(0), NULL);
				if (decl != NULL) {
					oomReport = 0;
				}
			...
			case OP_XML_ADD_NOTATION_DECL: {
				...
				decl = xmlAddNotationDecl(NULL, getDtd(1), getStr(0), getStr(1), getStr(2));
				if (decl != NULL) {
					oomReport = 0;
				}
				endOp();
			...
			case OP_XML_GET_DTD_ELEMENT_DESC: {
				...
				incNodeIdx();
				elem = xmlGetDtdElementDesc(getDtd(1), getStr(0));
				if (elem != NULL) {
					oomReport = 0;
				}
				/*
			...
			case OP_XML_GET_DTD_QELEMENT_DESC: {
				...
				incNodeIdx();
				elem      = xmlGetDtdQElementDesc(getDtd(1), getStr(0), getStr(1));
				oomReport = 0;
				if (elem != NULL && elem->parent == NULL) {
					elem = NULL;
				}
			...
			case OP_XML_GET_DTD_ATTR_DESC: {
				...
				incNodeIdx();
				decl = xmlGetDtdAttrDesc(getDtd(1), getStr(0), getStr(1));
				if (decl != NULL) {
					oomReport = 0;
				}
				setNode(0, (xmlNodePtr)decl);
			...
			case OP_XML_GET_DTD_QATTR_DESC: {
				...
				incNodeIdx();
				decl      = xmlGetDtdQAttrDesc(getDtd(1), getStr(0), getStr(1), getStr(2));
				oomReport = 0;
				setNode(0, (xmlNodePtr)decl);
				break;
			}
			...
			case OP_XML_GET_DTD_NOTATION_DESC:
				...
				xmlGetDtdNotationDesc(getDtd(1), getStr(0));
				oomReport = 0;
				endOp();
				break;
			...
			case OP_XML_ADD_ID:
				startOp("xmlAddID");
				xmlAddID(NULL, getDoc(0), getStr(0), getAttr(1));
				endOp();
				break;
			...
			case OP_XML_ADD_ID_SAFE: {
				...
				res       = xmlAddIDSafe(getAttr(0), getStr(0));
				oomReport = (res < 0);
				endOp();
				break;
			}
			...
			case OP_XML_GET_ID:
				startOp("xmlGetID");
				incNodeIdx();
				setNode(0, (xmlNodePtr)xmlGetID(getDoc(1), getStr(0)));
				oomReport = 0;
				break;
			...
			case OP_XML_IS_ID: {
				...
				startOp("xmlIsID");
				res       = xmlIsID(getDoc(2), getNode(1), getAttr(0));
				oomReport = (res < 0);
				endOp();
				break;
			}
			...
			case OP_XML_REMOVE_ID:
				startOp("xmlRemoveID");
				xmlRemoveID(getDoc(1), getAttr(0));
				oomReport = 0;
				endOp();
				break;
			...
			case OP_XML_ADD_REF: {
				...
				startOp("xmlAddRef");
				ref       = xmlAddRef(NULL, doc = getDoc(0), value = getStr(0), attr = getAttr(1));
				oomReport = (doc != NULL && value != NULL && attr != NULL && ref == NULL);
				endOp();
				break;
			}
			...
			case OP_XML_IS_REF:
				startOp("xmlIsRef");
				xmlIsRef(getDoc(2), getNode(1), getAttr(0));
				oomReport = 0;
				endOp();
				break;
			...
			case OP_XML_REMOVE_REF: {
				...
				res = xmlRemoveRef(getDoc(1), getAttr(0));
				if (res == 0) {
					oomReport = 0;
				}
				endOp();
			...
			case OP_XML_NEW_ENTITY: {
				...
				incNodeIdx();
				ent = xmlNewEntity(doc = getDoc(1), getStr(0), getInt(0), getStr(1), getStr(2), getStr(3));
				if (ent != NULL) {
					oomReport = 0;
				}
				if (doc == NULL || doc->intSubset == NULL) {
			...
			case OP_XML_ADD_ENTITY: {
				...
				incNodeIdx();
				res = xmlAddEntity(
					getDoc(1), getInt(0), getStr(0), getInt(1), getStr(1), getStr(2), getStr(3), &ent
				);
			...
			case OP_XML_ADD_DOC_ENTITY: {
				...
				incNodeIdx();
				ent = xmlAddDocEntity(getDoc(1), getStr(0), getInt(1), getStr(1), getStr(2), getStr(3));
				if (ent != NULL) {
					oomReport = 0;
				}
				setNode(0, (xmlNodePtr)ent);
			...
			case OP_XML_ADD_DTD_ENTITY: {
				...
				incNodeIdx();
				ent = xmlAddDtdEntity(getDoc(1), getStr(0), getInt(1), getStr(1), getStr(2), getStr(3));
				setNode(0, (xmlNodePtr)ent);
				break;
			}
			...
			case OP_XML_GET_PREDEFINED_ENTITY:
				...
				incNodeIdx();
				setNode(0, (xmlNodePtr)xmlGetPredefinedEntity(getStr(0)));
				oomReport = 0;
				break;
			...
			case OP_XML_GET_DOC_ENTITY:
				...
				incNodeIdx();
				setNode(0, (xmlNodePtr)xmlGetDocEntity(getDoc(1), getStr(0)));
				oomReport = 0;
				break;
			...
			case OP_XML_GET_DTD_ENTITY:
				...
				incNodeIdx();
				setNode(0, (xmlNodePtr)xmlGetDtdEntity(getDoc(1), getStr(0)));
				oomReport = 0;
				break;
			...
			case OP_XML_GET_PARAMETER_ENTITY:
				...
				incNodeIdx();
				setNode(0, (xmlNodePtr)xmlGetParameterEntity(getDoc(1), getStr(0)));
				oomReport = 0;
				break;
			...
			case OP_XML_ENCODE_ENTITIES_REENTRANT: {
				...
				incStrIdx();
				encoded   = xmlEncodeEntitiesReentrant(getDoc(0), string = getStr(1));
			...
			case OP_XML_ENCODE_SPECIAL_CHARS: {
				...
				incStrIdx();
				encoded   = xmlEncodeSpecialChars(getDoc(0), string = getStr(1));
				oomReport = (string != NULL && encoded == NULL);
				moveStr(0, encoded);
				endOp();
				break;
			...
			case OP_HTML_NEW_DOC: {
				...
				startOp("htmlNewDoc");
				incNodeIdx();
				doc       = htmlNewDoc(getStr(0), getStr(1));
				oomReport = (doc == NULL);
				setNode(0, (xmlNodePtr)doc);
				break;
			}
			...
			case OP_HTML_NEW_DOC_NO_DTD: {
				...
				incNodeIdx();
				doc       = htmlNewDocNoDtD(getStr(0), getStr(1));
				oomReport = (doc == NULL);
				setNode(0, (xmlNodePtr)doc);
				break;
			}
			...
			case OP_VALIDATE: {
				...
				switch (type) {
					...
					case XML_ELEMENT_DECL:
						res = xmlValidateElementDecl(vctxt, node->doc, (xmlElementPtr)node);
						break;
					case XML_ATTRIBUTE_DECL:
						res = xmlValidateAttributeDecl(vctxt, node->doc, (xmlAttributePtr)node);
						break;
			...
			case OP_XML_VALIDATE_DTD: {
				...
				res   = xmlValidateDtd(vctxt, getDoc(0), getDtd(1));
				if (res != 0) {
					oomReport = 0;
				}
				xmlFreeValidCtxt(vctxt);
			...
			case OP_HTML_DOC_DUMP_MEMORY_FORMAT: {
				...
				switch (op) {
					case OP_XML_DOC_DUMP_MEMORY:
						startOp("xmlDocDumpMemory");
						break;
					case OP_XML_DOC_DUMP_MEMORY_ENC:
						startOp("xmlDocDumpMemoryEnc");
						break;
					case OP_XML_DOC_DUMP_FORMAT_MEMORY:
						startOp("xmlDocDumpFormatMemory");
						break;
					case OP_XML_DOC_DUMP_FORMAT_MEMORY_ENC:
						startOp("xmlDocDumpFormatMemoryEnc");
						break;
					case OP_HTML_DOC_DUMP_MEMORY:
						startOp("htmlDocDumpMemory");
						break;
					case OP_HTML_DOC_DUMP_MEMORY_FORMAT:
						startOp("htmlDocDumpMemoryFormat");
						break;
				}
				...
				switch (op) {
					case OP_XML_DOC_DUMP_MEMORY:
						...
						break;
					case OP_XML_DOC_DUMP_MEMORY_ENC:
						xmlDocDumpMemoryEnc(doc, &out, &outSize, (const char*)getStr(1));
						break;
					case OP_XML_DOC_DUMP_FORMAT_MEMORY:
						xmlDocDumpFormatMemory(doc, &out, &outSize, getInt(0));
						break;
					case OP_XML_DOC_DUMP_FORMAT_MEMORY_ENC:
						xmlDocDumpFormatMemoryEnc(doc, &out, &outSize, (const char*)getStr(1), getInt(0));
						break;
	#ifdef LIBXML_HTML_ENABLED
					case OP_HTML_DOC_DUMP_MEMORY:
						...
						break;
					case OP_HTML_DOC_DUMP_MEMORY_FORMAT:
						htmlDocDumpMemoryFormat(doc, &out, &outSize, getInt(0));
						break;
			...
			case OP_HTML_NODE_DUMP: {
				...
				switch (op) {
					case OP_XML_NODE_DUMP:
						...
						break;
					case OP_XML_NODE_BUF_GET_CONTENT:
						startOp("xmlNodeBufGetContent");
						break;
					case OP_XML_ATTR_SERIALIZE_TXT_CONTENT:
						startOp("xmlAttrSerializeTxtContent");
						break;
					case OP_XML_DUMP_ELEMENT_DECL:
						startOp("xmlDumpElementDecl");
						break;
					case OP_XML_DUMP_ELEMENT_TABLE:
						startOp("xmlDumpElementTable");
						break;
					case OP_XML_DUMP_ATTRIBUTE_DECL:
						startOp("xmlDumpAttributeDecl");
						break;
					case OP_XML_DUMP_ATTRIBUTE_TABLE:
						startOp("xmlDumpAttributeTable");
						break;
					case OP_XML_DUMP_ENTITY_DECL:
						startOp("xmlDumpEntityDecl");
						break;
					case OP_XML_DUMP_ENTITIES_TABLE:
						startOp("xmlDumpEntitiesTable");
						break;
					case OP_XML_DUMP_NOTATION_DECL:
						startOp("xmlDumpNotationDecl");
						break;
					case OP_XML_DUMP_NOTATION_TABLE:
						startOp("xmlDumpNotationTable");
						break;
					case OP_HTML_NODE_DUMP:
						startOp("htmlNodeDump");
						break;
				}
				...
				switch (op) {
					...
					case OP_XML_ATTR_SERIALIZE_TXT_CONTENT:
						...
						xmlAttrSerializeTxtContent(buffer, doc, (xmlAttrPtr)node, getStr(1));
						break;
					case OP_XML_DUMP_ELEMENT_DECL:
					...
					case OP_XML_DUMP_ELEMENT_TABLE: {
						...
						table
							= node != NULL && node->type == XML_DTD_NODE ? ((xmlDtdPtr)node)->elements : NULL;
					...
					case OP_XML_DUMP_ATTRIBUTE_TABLE: {
						...
						table
							= node != NULL && node->type == XML_DTD_NODE ? ((xmlDtdPtr)node)->attributes : NULL;
					...
					case OP_XML_DUMP_NOTATION_TABLE: {
						...
						table
							= node != NULL && node->type == XML_DTD_NODE ? ((xmlDtdPtr)node)->notations : NULL;
					...
					case OP_XML_DUMP_ENTITIES_TABLE: {
						...
						table
							= node != NULL && node->type == XML_DTD_NODE ? ((xmlDtdPtr)node)->entities : NULL;
			...
			case OP_HTML_NODE_DUMP_FORMAT_OUTPUT: {
				...
				switch (op) {
					case OP_XML_SAVE_FILE_TO:
						startOp("xmlSaveFileTo");
						break;
					case OP_XML_SAVE_FORMAT_FILE_TO:
						startOp("xmlSaveFormatFileTo");
						break;
					case OP_XML_NODE_DUMP_OUTPUT:
						startOp("xmlNodeDumpOutput");
						break;
					case OP_HTML_DOC_CONTENT_DUMP_OUTPUT:
						startOp("htmlDocContentDumpOutput");
						break;
					case OP_HTML_DOC_CONTENT_DUMP_FORMAT_OUTPUT:
						startOp("htmlDocContentDumpFormatOutput");
						break;
					case OP_HTML_NODE_DUMP_OUTPUT:
						startOp("htmlNodeDumpOutput");
						break;
					case OP_HTML_NODE_DUMP_FORMAT_OUTPUT:
						startOp("htmlNodeDumpFormatOutput");
						break;
				}

				incStrIdx();
				output = xmlOutputBufferCreateIO(xmlFuzzOutputWrite, xmlFuzzOutputClose, NULL, NULL);
				xmlFuzzResetFailure();
				node     = getNode(0);
				doc      = node ? node->doc : NULL;
				...
				switch (op) {
					...
					case OP_XML_NODE_DUMP_OUTPUT:
						argsOk &= (node != NULL);
						xmlNodeDumpOutput(output, doc, node, level, format, encoding);
						break;
	#ifdef LIBXML_HTML_ENABLED
					case OP_HTML_DOC_CONTENT_DUMP_OUTPUT:
						argsOk &= (doc != NULL);
					...
					case OP_HTML_DOC_CONTENT_DUMP_FORMAT_OUTPUT:
						argsOk &= (doc != NULL);
						htmlDocContentDumpFormatOutput(output, doc, encoding, format);
						break;
					case OP_HTML_NODE_DUMP_OUTPUT:
						argsOk &= (node != NULL);
					...
					case OP_HTML_NODE_DUMP_FORMAT_OUTPUT:
						argsOk &= (node != NULL);
						htmlNodeDumpFormatOutput(output, doc, node, encoding, format);
						break;
	#endif /* LIBXML_HTML_ENABLED */
				}
			...
			case OP_XML_DOM_WRAP_RECONCILE_NAMESPACES: {
				...
				res = xmlDOMWrapReconcileNamespaces(NULL, node = getNode(0), getInt(0));
				oomReport
					= (node != NULL && node->doc != NULL && node->type == XML_ELEMENT_NODE && res < 0);
				endOp();
				break;
			}
			...
			case OP_XML_DOM_WRAP_ADOPT_NODE: {
				...
				res       = xmlDOMWrapAdoptNode(ctxt, doc, node, destDoc, destParent, getInt(0));
				if (ctxt == NULL) {
					oomReport = 1;
				} else if (res == 0) {
					oomReport = 0;
				...
				if (node != NULL) {
					...
					if (destParent != NULL && node->parent == NULL && node->doc == destParent->doc) {
						if (node->type == XML_ATTRIBUTE_NODE) {
							xmlNodePtr prop;
			...
			case OP_XML_DOM_WRAP_REMOVE_NODE: {
				...
				oomReport = (node != NULL && doc != NULL && node->doc == doc && res < 0);
				if (node != NULL && node->parent != oldParent) {
					if (fixNs(node) < 0) {
						oomReport = 1;
					}
			...
			case OP_XML_DOM_WRAP_CLONE_NODE: {
				...
				if (destParent != NULL && node != NULL && !isValidChildType(destParent, node->type)) {
					destParent = NULL;
				}

				/* xmlDOMWrapCloneNode returns a garbage node on error. */
				res = xmlDOMWrapCloneNode(ctxt, doc, node, &copy, destDoc, destParent, getInt(0), 0);
				if (ctxt == NULL) {
					oomReport = 1;
				} else if (res == 0) {
					oomReport = 0;
