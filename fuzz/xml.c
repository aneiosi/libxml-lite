...
int LLVMFuzzerTestOneInput(const char* data, size_t size) {
	...
	if (ctxt == NULL) {
		...
		if (doc != NULL) {
			...
			if (save == NULL) {
				...
				xmlFuzzCheckFailureReport(
					"xmlSaveToBuffer", saveErr == XML_ERR_NO_MEMORY, saveErr == XML_IO_EIO
				);
	...
	if (ctxt != NULL) {
		...
		while (numChunks == 0 || (consumed < docSize && numChunks < maxChunks)) {
			size_t chunkSize;
			int    terminate;
		...
		xmlFuzzCheckFailureReport(
			"xmlParseChunk", errorCodePush == XML_ERR_NO_MEMORY, errorCodePush == XML_IO_EIO
		);
		doc = ctxt->myDoc;
		 ...
		 */
		if (errorCode != XML_ERR_NO_MEMORY && errorCode != XML_IO_EIO
		    && errorCodePush != XML_ERR_NO_MEMORY && errorCodePush != XML_IO_EIO
		 ...
		 */
		if ((opts & XML_PARSE_NOBLANKS) == 0 && errorCode == XML_ERR_OK && errorCodePush == XML_ERR_OK
		    && outbuf != NULL) {
			...
			if (save != NULL) {
				...
				if (saveErr == XML_ERR_OK) {
					...
					if (outbufSize != xmlBufferLength(outbufPush)
					    || memcmp(xmlBufferContent(outbuf), xmlBufferContent(outbufPush), outbufSize) != 0) {
						fprintf(
							stderr,
							"pull/push parser roundtrip "
							"mismatch\n"
						);
		#if 0
                        FILE *f = fopen("c.xml", "wb");
...
size_t LLVMFuzzerCustomMutator(char* data, size_t size, size_t maxSize, unsigned seed) {
	...
	return xmlFuzzMutateChunks(chunks, data, size, maxSize, seed, LLVMFuzzerMutate);
}
