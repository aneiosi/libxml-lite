...
def run_test(
	...
	test_callback=None,
	root_name="root",
	root_content="replacement text",
):
	opts = libxml2.XML_PARSE_DTDLOAD | libxml2.XML_PARSE_COMPACT
	actual_err = []
	...
	if actual_status != exp_status:
		print(
			"Test '%s' failed: expect status '%s', actual '%s'"
			% (desc, exp_status, actual_status)
		)
		sys.exit(1)
...
run_test(
	desc="Loading entity without custom callback",
	docpath=startURL,
	catalog=None,
	exp_status="not loaded",
	exp_err=[
		(-1, "I/O "),
		(-1, "warning : "),
...
run_test(
	desc="Loading entity with custom callback",
	docpath=startURL,
	catalog=None,
	exp_status="loaded",
	exp_err=[
		(
			4,
			'failed to load "http://example.com/dtds/sample.dtd": Attempt to load network entity\n',
		),
...
run_test(
	desc="Loading entity with custom callback and catalog",
	docpath=startURL,
	catalog=catURL,
)
...
run_test(
	desc="Loading entity and unregistering callback",
	docpath=startURL,
	catalog=catURL,
...
run_test(
	desc="Retry loading document after unregistering callback",
	docpath=startURL,
	catalog=catURL,
	exp_status="not loaded",
	exp_err=[
...
run_test(
	desc="Loading using standard i/o after unregistering callback",
	docpath="tst.xml",
	catalog=None,
	root_name="doc",
	root_content="bar",
...
run_test(
	desc="Loading using standard i/o after unregistering all callbacks",
	docpath="tst.xml",
	catalog=None,
	exp_status="not loaded",
	exp_err=[
