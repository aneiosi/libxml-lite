...
void testXmlwriterFilename(const char* uri) {
	...
	writer = xmlNewTextWriterFilename(uri);
	if (writer == NULL) {
	...
	rc = xmlTextWriterWriteAttribute(writer, BAD_CAST "xml:lang", BAD_CAST "de");
	if (rc < 0) {
		printf("testXmlwriterFilename: Error at xmlTextWriterWriteAttribute\n");
		return;
	}
	...
	rc = xmlTextWriterWriteElement(writer, BAD_CAST "NAME_1", BAD_CAST "Mueller");
	if (rc < 0) {
	...
	rc = xmlTextWriterWriteElement(writer, BAD_CAST "NAME_2", BAD_CAST "Joerg");
	if (rc < 0) {
		printf("testXmlwriterFilename: Error at xmlTextWriterWriteElement\n");
		return;
	}
	...
	rc = xmlTextWriterWriteElement(writer, BAD_CAST "ARTICLE", BAD_CAST "<Test>");
	if (rc < 0) {
		printf("testXmlwriterFilename: Error at xmlTextWriterWriteElement\n");
		return;
	}
	...
	/* Write an element named "ARTICLE" as child of ENTRY. */
	rc = xmlTextWriterWriteElement(writer, BAD_CAST "ARTICLE", BAD_CAST "<Test 2>");
	if (rc < 0) {
		printf("testXmlwriterFilename: Error at xmlTextWriterWriteElement\n");
		return;
	}
	...
	rc = xmlTextWriterWriteElement(writer, BAD_CAST "TEXT", BAD_CAST "This is a text.");
	if (rc < 0) {
		printf("testXmlwriterFilename: Error at xmlTextWriterWriteElement\n");
		return;
	}
...
void testXmlwriterMemory(void) {
	...
	writer = xmlNewTextWriterMemory(buf);
	if (writer == NULL) {
...
void testXmlwriterDoc(void) {
	...
	writer = xmlNewTextWriterDoc(&doc);
	if (writer == NULL) {
...
void testXmlwriterTree(void) {
	...
	writer = xmlNewTextWriterTree(doc, node);
	if (writer == NULL) {
