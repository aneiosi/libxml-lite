...
static int  execute_xpath_expression(
	const char* filename,
 ...
 * @nsList: the optional list of known namespaces in "<prefix1>=<href1> <prefix2>=href2> ..." format.
 *
 ...
 * @nsList: the list of known namespaces in "<prefix1>=<href1> <prefix2>=href2> ..." format.
 *
	...
	while (next != NULL) {
		...
		*(next++) = '\0';
		...
		if (xmlXPathRegisterNs(xpathCtx, prefix, href) != 0) {
			fprintf(
				stderr, "Error: unable to register NS with prefix=\"%s\" and href=\"%s\"\n", prefix, href
			);
			xmlFree(nsListDup);
			return (-1);
		}
	...
	for (i = 0; i < size; ++i) {
		...
		if (nodes->nodeTab[i]->type == XML_NAMESPACE_DECL) {
			...
			if (cur->ns) {
				...
				fprintf(
					output, "= namespace \"%s\"=\"%s\" for node %s\n", ns->prefix, ns->href, cur->name
				);
			}
...
int main(int argc, char** argv) {
	...
	/* Do the main job */
	if (execute_xpath_expression(argv[1], BAD_CAST argv[2], (argc > 3) ? BAD_CAST argv[3] : NULL)
	    < 0) {
		usage(argv[0]);
		return (-1);
	}
