...
static int xmlValidateAttributeValueInternal(xmlAttributeType type, const xmlChar* value);
#endif
/************************************************************************
...
xmlAttribute* xmlAddAttributeDecl(
	...
	if ((ctxt != NULL) && (ctxt->flags & XML_VCTXT_VALIDATE) && (defaultValue != NULL)
	    && (!xmlValidateAttributeValueInternal(type, defaultValue))) {
		xmlErrValidNode(
			ctxt,
			(xmlNodePtr)dtd,
...
int xmlValidateAttributeValue(xmlAttributeType type, const xmlChar* value) {
	return (xmlValidateAttributeValueInternal(type, value));
}
...
int xmlValidateAttributeDecl(xmlValidCtxt* ctxt, xmlDoc* doc, xmlAttribute* attr) {
	...
	/* Enumeration */
	if (attr->defaultValue != NULL) {
		val = xmlValidateAttributeValueInternal(attr->atype, attr->defaultValue);
		if (val == 0) {
			xmlErrValidNode(
				ctxt,
				(xmlNodePtr)attr,
...
int xmlValidateOneAttribute(
	...
	val         = xmlValidateAttributeValueInternal(attrDecl->atype, value);
	if (val == 0) {
		xmlErrValidNode(
			ctxt,
			elem,
