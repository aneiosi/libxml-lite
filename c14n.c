 ...
 * @param is_visible_callback: the function to use to determine is node visible or not
 * @param user_data: the first parameter for `is_visible_callback` function (in most cases, it is
 * nodes set)
 * @param mode: the c14n mode (see `xmlC14NMode`)
 * @param inclusive_ns_prefixes: the list of inclusive namespace prefixes ended with a NULL, or
 * NULL if there is no inclusive namespaces (only for canonicalization)
 * @param with_comments: include comments in the result (!=0) or not (==0)
 * @param buf: the output buffer to store canonical XML; this buffer MUST have encoder==NULL
 * because C14N requires UTF-8 output
 ...
 * nodes set)
 * @param mode  the c14n mode (see `xmlC14NMode`)
 * @param inclusive_ns_prefixes  the list of inclusive namespace prefixes ended with a NULL or NULL
 * if there is no inclusive namespaces (only for exclusive canonicalization, ignored otherwise)
 * @param with_comments  include comments in the result (!=0) or not (==0)
 * @param buf  the output buffer to store canonical XML; this buffer MUST have encoder==NULL
 * because C14N requires UTF-8 output
